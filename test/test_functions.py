...
class TestMultiHeadAttentionForward:
    ...
    @pytest.mark.parametrize("dtype", [torch.float64, torch.float32])
    @pytest.mark.parametrize("need_weights", [True, False])
    def test_fully_masked_query_row(self, dtype, need_weights):
        # A batch element whose entire key sequence is padded: need_weights=False
        # must match torch's safe softmax (zero output), while need_weights=True
        # matches torch's explicit softmax (NaN). npt handles equal NaNs.
        torch.manual_seed(0)
        q, k, v = self._qkv(dtype)
        kpm = torch.zeros(self.N, self.S, dtype=torch.bool)
        kpm[1, :] = True
        self._compare(
            q,
            k,
